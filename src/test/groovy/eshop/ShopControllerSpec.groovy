...
class ShopControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.shopService = Mock(ShopService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.shopService = Mock(ShopService) {
            1 * save(_ as Shop) >> { Shop shop ->
                throw new ValidationException("Invalid instance", shop.errors)
            }
        }
